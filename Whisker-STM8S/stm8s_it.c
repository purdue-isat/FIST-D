...
extern char loop_sensor;
 INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
{
    /* Read one byte from the receive data register */
    u8 x = UART1_ReceiveData8();
		if(uartstate==0 && x== BOARD_ID) uartstate=1;
		if(uartstate==0 && x>=AUTOSTART_ID && x<=AUTOSTART_ID+10) {
			loop_sensor=x-AUTOSTART_ID;
		}
		if(uartstate==0 && x== BOARD_id) uartstate=2;
